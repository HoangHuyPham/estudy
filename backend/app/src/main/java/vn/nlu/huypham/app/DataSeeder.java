...
public class DataSeeder implements CommandLineRunner
{
		...
		String... args)
	{
		...
		createUserIfNotExists("Admin User", "admin", "admin@estudy.local", "admin123", adminRole);

		createUserIfNotExists("Regular User", "student", "student@estudy.local", "student123",
				userRole);

		createUserIfNotExists("Tutor User", "tutor", "tutor@estudy.local", "tutor123", tutorRole);
	}
		...
		Roles name)
	{
		...
		return roleRepo.save(Role.builder().name(name).build());
	}
		...
		Role role)
	{
		...
		userRepo.save(User.builder().displayName(displayName).username(username).email(email)
				.password(passwordEncoder.encode(rawPassword)).isEnabled(true)
				.isAccountNonLocked(true).roles(Set.of(role)).build());
	}
}
