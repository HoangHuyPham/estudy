...
public class AuthServiceImp implements AuthService
{
		...
		String password) throws AppException
	{
		try
		{
			...
			return TokenPairResponse.builder()
					...
					.refreshToken(jwtService.generateRT(userPrincipal.getUser())).build();
		}
		...
		String googleIDToken) throws AppException
	{
		try
		{
			...
			{
				User newUser = User.builder().email(email).username(email).displayName(name)
						.avatar(picture).isEnabled(true)
						.roles(Set.of(roleRepo.findByName(Roles.STUDENT)
								.orElseThrow(() -> Errors.AUTH_FAILED)))
						.isAccountNonLocked(true).build();
				return userRepo.save(newUser);
			});

			return TokenPairResponse.builder().accessToken(jwtService.generateAT(user))
					.refreshToken(jwtService.generateRT(user)).build();
		}
		catch (Exception e)
		{
			log.error("Error during authentication: {}", e);
		...
		String otp) throws AppException
	{
		...
		user.setRoles(Set
				.of(roleRepo.findByName(Roles.STUDENT).orElseThrow(() -> Errors.REGISTER_FAILED)));
		userRepo.save(user);
