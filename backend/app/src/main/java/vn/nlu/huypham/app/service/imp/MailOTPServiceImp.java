...
public class MailOTPServiceImp implements MailOTPService
{
		...
		TextEmailContent content)
	{
		try
		{
			...
			String html = FreeMarkerTemplateUtils.processTemplateIntoString(template, Map.of(
					"title", subject, "name", content.getName(), "content", content.getContent()));
		...
		ClickableEmailContent content)
	{
		try
		{
			...
			String html = FreeMarkerTemplateUtils.processTemplateIntoString(template,
					Map.of("title", subject, "content", content.getContent(), "name",
							content.getName(), "buttonText", content.getButtonText(), "buttonUrl",
							content.getButtonUrl()));
			sendHtmlMail(to, subject, html);
		}
		catch (Exception e)
		{
		...
		OTPEmailContent content)
	{
		try
		{
			...
			String html = FreeMarkerTemplateUtils.processTemplateIntoString(template,
					Map.of("content", content.getContent(), "title", subject, "name",
							content.getName(), "otp", content.getOtp()));
			sendHtmlMail(to, subject, html);
		}
		catch (Exception e)
		{
	...
	@Override
	public MailOTP createRegisterOTP(
		RegisterRequest dto)
	{
		...
		MailOTP mailOTP = MailOTP.builder().isUsed(false)
				.otp("%06d".formatted((int) (Math.random() * 999999))).payload(payload)
				.type(OTPTypes.REGISTER_ACCOUNT)
				.expiredAt(
						Instant.now().plusSeconds(appConfig.getMail().getOtpExp()).toEpochMilli())
				.build();
	...
	@Override
	public User validateOTP(
		UUID mailOTPId,
		String email,
		String otp)
	{
		MailOTP mailOTP = mailOTPRepo.findById(mailOTPId)
				.orElseThrow(() -> Errors.MAIL_OTP_NOT_FOUND);
		...
		User user = User.builder().displayName((String) mailOTP.getPayload().get("displayName"))
				...
				.avatar((String) mailOTP.getPayload().get("avatar")).isEnabled(true)
